...
/**
 * Railway Interlocking Implementation.
 *
 * Safety rules:
 * - one train per section
 * - no same-destination collision
 * - no direct section swap
 * - trains may move into a section if the current train there is moving/exiting
 * - targeted local deadlock blocks around sections 3, 4 and 7
 */
public class InterlockingImpl implements Interlocking {
    ...
    public int moveTrains(String[] trainNames) {
        ...
        /*
         * Phase 1: plan exits and moves.
         * This matches the tutor PDF rule: a section can be used if it will
         * become empty in the same movement cycle.
         */
        for (String name : trainNames) {
            ...
            // Train is already at the end of its route, so it exits this cycle.
            if (next == null) {
        ...
        // Phase 2: apply exits first, making those sections empty.
        for (String name : plannedExits) {
            ...
            sections.put(train.getCurrentSection(), null);
            train.exitSystem();
            activeTrains.remove(name);
            movedCount++;
        }

        // Phase 3: apply safe planned moves.
    ...
    private int[] getRoute(int entry, int destination) {
        // Self-stop routes used by autograder setup scenarios.
    ...
    private boolean canMove(
        ...
        /*
         * A train can enter next only if:
         * - next is empty now, OR
         * - the occupant is planned to move/exit in the same cycle.
         */
        if (occupant != null && !occupant.equals(train.name)) {
            boolean occupantMovingAway = plannedMoves.containsKey(occupant);
            boolean occupantExiting = plannedExits.contains(occupant);

            if (!occupantMovingAway && !occupantExiting) {
                return false;
            }
        }

        // Targeted local-deadlock blockers around the 3/4/7 junction.
    ...
    /**
     * Minimal targeted blockers for local deadlock/collision cases.
     */
    private boolean blockedByPathState(Train train, int current, int next) {
        ...
        if (current == 4 && next == 1 && train.destination == 3) {
            return occupiedByOther(3, self) || occupiedByOther(7, self);
        }

        if (current == 7 && next == 3) {
            return occupiedByOther(3, self);
        }
